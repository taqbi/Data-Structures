...
{ 
	...
	// We always need j trials for one egg and j floors. 
		...
		{ 
			for (j = 2; j <= k; j++) 
			{ 
				eggFloor[i][j] = Integer.MAX_VALUE; //Fills the entry with infinity
				for (x = 1; x <= j; x++) 
				{ 
	...
	{ 
		...
		System.out.println("Enter the value for number of eggs:  ");
		n = sc.nextInt();
		System.out.println("Enter the value for number of floors:  ");
		k = sc.nextInt();
